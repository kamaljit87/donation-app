...
// Note: Metadata should be in a separate metadata export or layout
// For now, we'll use next/head in the component
...
const DonatePage = () => {
        ...
        handler: async (response) => {
          try {
            ...
            if (verifyData.success) {
              ...
              router.push(`/thank-you?amount=${formData.amount}`);
            } else {
        ...
        modal: {
          ondismiss: async () => {
            await fetch('/api/payment/failed', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ order_id: orderData.data.order_id, dismissed: true }),
            });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        await handlePayment(result.data.donation_id, formData.amount);
      } else {
